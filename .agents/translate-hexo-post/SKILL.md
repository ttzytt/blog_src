...
10. Compare source and target for completeness, fidelity, structure, protected syntax, glossary compliance, configured character ratio, and exact code-fence/image/link counts.
11. Run the taxonomy, front matter consistency, and content completeness checkers immediately after translation. Run the language-coverage checker and the repository's build command when available and proportionate. Report any validation not run.
...
Treat the repository's directional language-pair conversion coefficient as a mandatory completeness gate, not as a translation target. Calculate `minimum target characters = source characters * conversion coefficient`. For the current alphanumeric prose counter, use the conservative lower bounds `1.5` for `zh-CN -> en` and `1 / 1.5` for `en -> zh-CN`; therefore, 1,000 Chinese prose characters require at least 1,500 English prose characters. Exclude front matter, the machine warning, fenced code, URLs, and syntax. Never pad a translation to pass: a low count requires checking for omitted or over-compressed content and translating it faithfully.

Preserve exactly the source count of:

- Fenced code blocks.
- Rendered images.
- Rendered links, excluding the mandatory warning's source-language link.

Do not merge, delete, or invent these structures. Preserve every code block in its original position and translate comments according to the code rules. Preserve image destinations and link destinations; translate only their reader-visible labels or alt text.
...
- The configured target/source prose character ratio meets its minimum without padding or elaboration.
...
- The content completeness validator passes for the new pair.
...
3. Run the content completeness checker after every translated file. Investigate every length or structure mismatch; never add filler, remove source structures, or weaken the configured coefficient to force a pass.
4. Run the language-coverage checker in warning mode while translation is incomplete.
5. Run coverage in strict mode once all configured language trees are expected to be complete and in CI.
6. Run the composed multilingual build when the change can affect rendering, routes, Hexo tags, or assets.
