---
title: Treap（树堆）学习笔记，基于洛谷 P3369
date: 2022-06-13 22:56:57
alias:
  - "2022/06/treap_note/index.html"
...
我并不清楚如何去严格的证明这样随机化的过程可以让搜索树的复杂度的**期望值**保持在 $\log_2{n}$，但我们可以试着感性的去理解一下。
...
<!-- 如果想要学习数组实现，可以拉到最下面的完整代码部分。 -->

{% note info %}
注意本代码中的 `rank` 代表前面讲的 $\textit{priority}$ 变量（堆的值）。并且，维护的堆的性质是小根堆（$\textit{priority}$ 小的在上面）。
{% endnote %}

## 具体实现
本文的树堆实现了[洛谷模板题](https://www.luogu.com.cn/problem/P3369)中的操作，因为是在竞赛中使用的，所以没有用模板一类的东西封装。

下面的代码也大量参考了[这篇文章](https://article.itxueyuan.com/dRlRJ)。
...
下图展示了 $\textit{cur}$ 的值小于等于 $\textit{key}$ 时按值分裂的情况。[^2]
...
翻转 $[l, r]$ 这个区间时，基本思路是将树分裂成 $[1, l - 1],\ [l, r],\ [r + 1, n]$ 三个区间，再对中间的 $[l, r]$ 进行翻转[^4]。
...
class Treap {
   private:
        ...
        if (val > cur->val) {
            ...
            switch (state) {
                ...
                case 3:
                    ...
                    // dir 也是更小的那个儿子
                    _rotate(cur, dir);  //这里的旋转可以把优先级更小的儿子转上去
                    //旋转完成后，原来的根节点就在旋转方向的那边
                    _del(cur->ch[!dir], val);
                    cur->upd_siz();
                    break;
            }
    ...
    int _query_rank(Node *cur, int val) {
        //查询以 cur 为根节点的子树中，val 这个值的大小的排名 （该子树中小于 val
        //的树的个数 + 1）
        int less_siz = cur->ch[0] == nullptr ? 0 : cur->ch[0]->siz;
        //这个树中小于 val 的节点的数量
        if (val == cur->val)
            ...
            if (cur->ch[0] != nullptr)
                ...
                return 1;  //如果说比最小的节点还要小，那这个数字就是最小的
        } else {
            if (cur->ch[1] != nullptr)
                return less_siz + cur->rep_cnt + _query_rank(cur->ch[1], val);
            else
...
<!-- endtab -->
{% endtabs %}
[^2]: 本图的设计参考了 [维基百科 treap 词条的配图](https://en.wikipedia.org/wiki/Treap)
